...
// Arc Testnet RPC can silently fail on wide block
// ranges even within the 9,999-block cap. Chunking
// into 500-block windows avoids silent drops under
// rate limits and keeps each eth_getLogs call small.
// ═══════════════════════════════════════════════
async function queryFilterChunked(contract, filter, from, to, chunkSize = 500) {
  ...
  for (let start = from; start <= to; start += chunkSize) {
    ...
    try {
      ...
      console.warn(`queryFilterChunked: chunk [${start}–${end}] failed, skipping.`, e);
    }
  }
  return events;
}
...
async function loadJobsFromChain() {
  try {
    ...
    const events = await queryFilterChunked(contract, contract.filters.JobCreated(), from, latest, 500);
